...
export class MonthHeatmap {
	...
	constructor(parent: HTMLElement, options?: MonthHeatmapOptions) {
		this.container = parent.createDiv('tg-module tg-month-heatmap');
		...
		header.createSpan({ text: '本月', cls: 'tg-module-title' });
		...
		// 网格容器
		this.grid = this.container.createDiv('tg-month-grid');
	}
	...
	update(now: Date): void {
		...
		weekLabels.forEach(label => {
			const labelEl = this.grid.createDiv('tg-week-label');
			labelEl.textContent = label;
		});

		// 填充空白（月初之前的日期）
		for (let i = 0; i < startOffset; i++) {
			this.grid.createDiv('tg-month-day-cell empty');
		}

		// 创建日期格子
		for (let day = 1; day <= daysInMonth; day++) {
			const cell = this.grid.createDiv('tg-month-day-cell');
